    ...
    <div x-data="{ open: @entangle('showEdit') }">
        <x-modal confirmText="{{ __('Update') }}" action="update">
            ...
            <div class="gap-4 mt-5 border-t">
                {{-- with initial emit --}}
                {{-- delivery boy --}}
                <livewire:component.autocomplete-input title="{{ __('Delivery Boy') }}" placeholder="{{ __('Search for driver') }}" column="name" model="User" customQuery="fleet-driver" initialEmit="preselectedDeliveryBoyEmit" emitFunction="autocompleteDriverSelected" onclearCalled="clearAutocompleteFieldsEvent" />
