<label class="block mt-4 text-sm">
        ...
        <div
            ...
            <div x-show="!isUploading">
                ...
                {{-- Form File picker --}}
                <input type="file" class="hidden" accept="{{ $fileTypes ?? '' }}" {{ ($multiple ?? false) ? 'multiple':'' }}
                    @if ( $defer ?? true )
                                ...
                                @if ($photo != null)      
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div class="font-light text-gray-500">
                                            ...
                                            <button wire:key="remove-photo-{{ rand(10,3004) }}" type="button" wire:click="removePhoto({{ $key }})"  class="px-2 mt-2 text-xs text-red-400 border border-red-400 rounded">
                                                {{__('Remove')}}
                                            </button>
                                        </div>
                                    </div>
