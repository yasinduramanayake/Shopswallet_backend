...
<div>
    ...
    <div x-data="{ open: @entangle('showCreate') }">
        <x-modal confirmText="{{ __('Save') }}" action="save">
            ...
            <x-select :options="$vendorTypes" name="vendor_type_id" title="{{ __('Vendor Type') }}" noPreSelect="true" :defer="false" />
            <x-input type="datetime-local" name="expires_at" title="{{ __('Expires At') }}" />

            {{-- search bar  --}}
            <x-autocomplete-input title="{{ __('Items/Products') }}" placeholder="{{ __('Enter search keyword') }}" :dataList="$searchResult" name="search_input" emitFunction="productSelected" />
            {{-- items  --}}
