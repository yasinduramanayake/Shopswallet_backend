    ...
    <x-baseview title="">
        ...
        <div class="flex items-center">
            ...
            <div class="flex items-center space-x-2">
                ...
                <a href="{{ route('users.details',$prevUserId) }}" class="text-white bg-gray-500 rounded-full hover:text-gray-300 hover:bg-gray-700" title="{{ __('Prev User') }}">
                    <x-heroicon-o-arrow-left class="w-8 h-8 p-2" />
                </a>
                @endif
                @if (!empty($nextUserId))
                <a href="{{ route('users.details',$nextUserId) }}" class="text-white bg-gray-500 rounded-full hover:text-gray-300 hover:bg-gray-700" title="{{ __('Next User') }}">
        ...
        <div class="grid gap-6 mt-8 md:grid-cols-2 lg:grid-cols-4">
            ...
            <x-dashboard-card-plain bg="bg-primary-500 text-white border-primary-500">
                ...
                @else
                <p>{{ \Str::padLeft('', Str::of($selectedModel->email ?? '')->length(), '*') }} <span>|</span> {{ \Str::padLeft('', Str::of($selectedModel->phone ?? '')->length(), '*') }}</p>
                @endproduction
            </x-dashboard-card-plain>
            {{-- wallet balance  --}}
            <x-dashboard-card bg="bg-primary-100" title="{{ __('Wallet Balance') }}" value="{{ currencyformat($selectedModel->wallet->balance ?? 0.00) }}">
                <x-lineawesome-wallet-solid class="w-16 " />
            </x-dashboard-card>

            {{-- loyalty point  --}}
            @if ((bool) setting('finance.enableLoyalty', false))
            <x-dashboard-card bg="bg-primary-100" title="{{ __('Loyalty Points') }}" value="{{ $loyaltyPoints ?? 0.00 }}">
                <x-lineawesome-gifts-solid class="w-16 " />
            </x-dashboard-card>
            @endif

            {{-- Orders --}}
            <x-dashboard-card bg="bg-primary-500 text-white border-primary-500" title="{{ __('Total Orders') }}" value="{{ $ordersCount ?? 0 }}">
                <x-heroicon-s-shopping-bag class="w-16 text-white" />
            </x-dashboard-card>
            {{-- most expensive order  --}}
            @if (!(bool) setting('finance.enableLoyalty', false))
            <x-dashboard-card bg="bg-primary-100" title="{{ __('Most Expensive Order') }}" value="{{ currencyformat($expensiveOrders ?? 0) }}">
